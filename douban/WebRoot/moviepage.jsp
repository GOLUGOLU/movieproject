<%@ page language="java" import="java.util.*,com.neu.cxl.Entity.Resource" pageEncoding="utf-8"%>
...
if(session=="null")
{
	...
	document.getElementById("login_review").innerHTML="亲，要<a href='login.html'>登录</a>后才可以评论哦!"
}
else
{
...
function publish()
  {
  window.open("publishreview.jsp?movieid=${requestScope.movie.getMovieId()}&moviename=${requestScope.movie.getMovieName()}&reviewuser=${sessionScope.user.getUserName()}",'短评','height=300,width=520,top=0,left=0,toolbar=no,menubar=no,scrollbars=no,resizable=no,location=no,status=no');
